...
const IconGithub = (props) => (
  ...
  >
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3-1 6-4 6-8s-2-8-6-8c-1 0-4.9.4-7.9 3.5a1.5 1.5 0 0 0-1 1.5c.3 1.2 0 2.5-.3 3.5-3 4-3 7-3 8-1 2-2 3-2 3" />
    <path d="M9 18c-3 1-3 1-4 2" />
  </svg>
);
...
// --- Utility Function (Replaces clsx/tailwind-merge) ---
function cn(...inputs) {
  // A simple merge utility for demonstration purposes
...
const SOCIAL_LINKS = [
  { id: 1, label: "GitHub", href: "https://github.com/yourusername", icon: <IconGithub size={24} /> },
  { id: 2, label: "LinkedIn", href: "https://linkedin.com/in/yourusername", icon: <IconLinkedin size={24} /> },
  { id: 3, label: "Email", href: "mailto:youremail@example.com", icon: <IconMail size={24} /> },
];
// --- End Mock Data ---
...
const BackgroundBeams = React.memo(({ className }) => {
  return (
    <div
      className={cn(
        "absolute inset-0 flex h-full w-full items-center justify-center [mask-repeat:no-repeat] [mask-size:40px] opacity-70",
        className
      )}
    >
      {/* SVG Container for the Beams */}
      <svg
        ...
        xmlns="http://www.w3.org/2000/svg"
      >
        {/* Base Path (Invisible grid structure) */}
        <path
          d={BACKGROUND_PATHS.join(" ")}
        ...
        <defs>
          {BACKGROUND_PATHS.map((path, index) => (
            ...
            >
              ...
              <stop offset="100%" stopColor="#AE48FF" stopOpacity="0.05"></stop>
            </motion.linearGradient>
          ))}

          {/* Radial Gradient for the background mask effect */}
          <radialGradient
...
// --- FOOTER COMPONENT (Main export) ---

const Footer = ({ portfolioName = "Developer" }) => {
  const currentYear = new Date().getFullYear();

  return (
    // Fixed height container for the footer section
    <div
    ...
    >
      <div className="fixed bottom-0 left-0 h-[550px] w-full"> 
        <footer
          className="relative flex h-full w-full flex-col justify-between bg-neutral-950 text-white"
          role="contentinfo"
        >
          
          {/* Background: Background Beams Effect */}
          <BackgroundBeams />

          {/* Content Container (z-index ensures it sits above the background beams) */}
          <div
            className="relative flex flex-grow items-center px-4 md:px-8 lg:px-10"
            style={{ zIndex: 10 }} // Ensure content is clearly on top
          >
                  ...
                  <div className="mb-8 flex gap-4 md:justify-end">
                    {SOCIAL_LINKS.map((social) => (
                      <motion.a
                        ...
                        className="flex h-12 w-12 items-center justify-center rounded-full bg-neutral-800 text-white transition-all duration-200 hover:bg-cyan-500 hover:text-neutral-950"
